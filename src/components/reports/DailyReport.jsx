...
const DailyReport = ({ transactions }) => {
  ...
  const dailyData = useMemo(() => {
    ...
    Object.keys(dataByDay).forEach((date) => {
      ...
      data.lucroLiquido = data.vendas - data.despesas;
      data.margem =
        data.vendas > 0 ? (data.lucroLiquido / data.vendas) * 100 : 0;
      data.ticketMedio =
        data.transacoes > 0
          ? (data.vendas + data.compras + data.despesas) / data.transacoes
          : 0;
      data.materiaisCount = data.materiais.size;
      data.eficiencia =
        data.transacoes > 0 ? data.lucroLiquido / data.transacoes : 0;
    });
  ...
  const periodStats = useMemo(() => {
    ...
    const melhorDia = allData.reduce(
      (max, day) => (day.lucroLiquido > max.lucroLiquido ? day : max),
      {
        lucroLiquido: Number.NEGATIVE_INFINITY,
      },
    );
    const piorDia = allData.reduce(
      (min, day) => (day.lucroLiquido < min.lucroLiquido ? day : min),
      {
    ...
    return {
      ...
      melhorDia:
        melhorDia.lucroLiquido !== Number.NEGATIVE_INFINITY ? melhorDia : null,
      piorDia:
        piorDia.lucroLiquido !== Number.POSITIVE_INFINITY ? piorDia : null,
      mediaVendasDia:
        totalDias > 0
          ? allData.reduce((sum, d) => sum + d.vendas, 0) / totalDias
          : 0,
      mediaLucroDia:
        totalDias > 0
          ? allData.reduce((sum, d) => sum + d.lucroLiquido, 0) / totalDias
          : 0,
    };
  }, [dailyData]);
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6 p-4 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-lg">
        ...
        <div className="text-center">
          ...
          <p className="text-xl font-bold text-green-600">
            {periodStats.percentualLucro.toFixed(1)}%
          </p>
          <p className="text-xs text-gray-500">
            {periodStats.diasComLucro} dias lucrativos
          </p>
        </div>
        <div className="text-center">
          ...
          <p className="text-xl font-bold text-purple-600">
            {formatCurrency(periodStats.mediaVendasDia)}
          </p>
        </div>
        <div className="text-center">
          ...
          <p
            className={`text-xl font-bold ${periodStats.mediaLucroDia >= 0 ? "text-green-600" : "text-red-600"}`}
          >
      ...
      {dailyData.length === 0 ? (
        <div className="text-center py-12">
          ...
          <p className="text-sm text-gray-400">
            Adicione transações para visualizar a análise diária
          </p>
        </div>
      ) : (
            ...
            >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
                <div className="text-center p-4 bg-green-50 rounded-lg border border-green-100">
                  ...
                  <p className="text-xs text-gray-500">
                    {data.transacoesVenda} transações
                  </p>
                </div>
                ...
                <div className="text-center p-4 bg-blue-50 rounded-lg border border-blue-100">
                  ...
                  <p className="text-xs text-gray-500">
                    {data.transacoesCompra} transações
                  </p>
                </div>
                ...
                <div className="text-center p-4 bg-red-50 rounded-lg border border-red-100">
                  ...
                  <p className="text-xs text-gray-500">
                    {data.transacoesDespesa} lançamentos
                  </p>
                </div>
                ...
                >
                  ...
                  <p
                    className={`text-lg font-bold ${data.lucroLiquido >= 0 ? "text-emerald-600" : "text-orange-600"}`}
                  >
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pt-4 border-t border-gray-200">
                ...
                <div className="text-center">
                  ...
                  <p
                    className={`font-semibold ${data.eficiencia >= 0 ? "text-green-600" : "text-red-600"}`}
                  >
