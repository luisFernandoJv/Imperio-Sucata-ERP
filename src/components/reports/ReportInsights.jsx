...
const InsightCard = ({ icon: Icon, title, description, type = "info", trend }) => {
  const typeConfig = {
    success: {
  ...
  return (
    ...
    >
      <div className="flex items-start gap-3">
        <div className={`${config.iconBg} p-2 rounded-lg shadow-md flex-shrink-0`}>
          <Icon className="h-5 w-5 text-white" />
        </div>
        <div className="flex-1 min-w-0">
          <h4 className={`font-bold text-sm mb-1 ${config.textColor}`}>{title}</h4>
  ...
  return (
    <div className="bg-white rounded-xl p-4 shadow-md border-2 border-gray-100 hover:shadow-lg transition-all">
      <div className="flex items-center justify-between mb-2">
        <div className={`bg-gradient-to-br ${colorConfig[color]} p-2 rounded-lg shadow-md`}>
...
const ReportInsights = ({ transactions, stats, dateRange }) => {
  const insights = useMemo(() => {
    ...
    transactions.forEach((t) => {
      ...
      materialStats[t.material].lucro = materialStats[t.material].vendas - materialStats[t.material].compras
    })
    ...
    const busiestDay = Object.entries(dayStats).sort((a, b) => b[1].count - a[1].count)[0]
    ...
    Object.entries(avgPurchasePrice).forEach(([material, data]) => {
      if (data.prices.length >= 3) {
        ...
        const recent = sorted.slice(-3).reduce((sum, p) => sum + p.price, 0) / 3
        const older = sorted.slice(0, -3).reduce((sum, p) => sum + p.price, 0) / Math.max(1, sorted.length - 3)
    ...
    // KPIs
    const ticketMedio =
      stats.totalTransactions > 0 ? (stats.totalSales + stats.totalPurchases) / stats.totalTransactions : 0
    const margemLucro = stats.totalSales > 0 ? (stats.totalProfit / stats.totalSales) * 100 : 0
    const roi = stats.totalPurchases > 0 ? (stats.totalProfit / stats.totalPurchases) * 100 : 0
    const diasPeriodo = Math.ceil((dateRange.to - dateRange.from) / (1000 * 60 * 60 * 24)) || 1
    ...
    <div className="space-y-6">
      ...
      <Card className="p-6 bg-gradient-to-br from-white to-slate-50 shadow-xl border-2 border-slate-200">
        <div className="flex items-center gap-3 mb-6">
          ...
          <h3 className="text-2xl font-bold text-slate-900">Indicadores de Performance (KPIs)</h3>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <KPICard
            ...
            subtitle={
              insights.kpis.margemLucro >= 30 ? "Excelente" : insights.kpis.margemLucro >= 15 ? "Boa" : "Melhorar"
            }
            icon={Target}
            color={insights.kpis.margemLucro >= 30 ? "green" : insights.kpis.margemLucro >= 15 ? "orange" : "red"}
          />
          <KPICard
            label="ROI"
      ...
      <Card className="p-6 bg-gradient-to-br from-white to-blue-50 shadow-xl border-2 border-blue-200">
        <div className="flex items-center gap-3 mb-6">
          ...
          <h3 className="text-2xl font-bold text-slate-900">Insights Inteligentes</h3>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          {insights.highestMargin && insights.highestMargin.material !== insights.mostProfitable?.material && (
            <InsightCard
              icon={Zap}
      ...
      {insights.costIncreaseAlert && (
        <Card className="p-6 bg-gradient-to-br from-yellow-50 to-amber-50 shadow-xl border-2 border-yellow-300">
          <div className="flex items-center gap-3 mb-4">
            ...
            <h3 className="text-2xl font-bold text-yellow-900">Alertas Importantes</h3>
          </div>
            ...
            <div className="flex items-center gap-2">
              {stats.totalProfit >= 0 ? (
                <>
                  ...
                  <span className="text-lg font-bold text-green-600">Lucrativo</span>
                </>
              ) : (
                <>
                  <TrendingDown className="h-5 w-5 text-red-600" />
                  <span className="text-lg font-bold text-red-600">Prejuízo</span>
                </>
              )}
            </div>
          </div>
            ...
            <div className="flex items-center gap-2">
              {insights.kpis.margemLucro >= 20 ? (
                ...
                <>
                  ...
                  <span className="text-lg font-bold text-yellow-600">Média</span>
                </>
              ) : (
                <>
