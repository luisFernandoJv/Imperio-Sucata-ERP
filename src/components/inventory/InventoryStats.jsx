  ...
  return (
    <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm hover:shadow-md transition-all duration-300 relative overflow-hidden group">
      <div className="flex justify-between items-start relative z-10">
        ...
        <div className={`p-3 rounded-xl ${theme} transition-transform group-hover:scale-110`}>
          <Icon className="h-6 w-6" />
        </div>
      </div>
      {/* Background Decorativo Sutil */}
      <div className={`absolute -bottom-6 -right-6 opacity-5 pointer-events-none transform rotate-12 group-hover:scale-125 transition-transform duration-500`}>
...
export const InventoryStats = memo(({ inventory, materials }) => {
  const stats = useMemo(() => {
    ...
    const margin = totalValue > 0 ? ((potentialRevenue - totalValue) / totalValue) * 100 : 0
