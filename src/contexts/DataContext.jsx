...
export const DataProvider = ({ children }) => {
  ...
  useEffect(() => {
    ...
    try {
      unsubscribeTransactions = subscribeToTransactions((firebaseTransactions) => {
        console.log(`[v0] Recebidas ${firebaseTransactions.length} transações do listener (LIMITADO).`)
      ...
      unsubscribeLiveSummary = subscribeToLiveSummary((summary) => {
        console.log("[v0] Live summary atualizado (1 leitura):", summary ? "OK" : "vazio")
  ...
  const refreshData = async () => {
    ...
    try {
      ...
      const [firebaseTransactions, firebaseInventory, summary] = await Promise.all([
        getTransactions(),
        getInventory(),
        getLiveSummary(),
      ])
  ...
  const fetchTransactionsByPeriod = async (startDate, endDate) => {
    try {
      console.log("[v0] Fetching transactions by period:", { startDate, endDate })
