...
export async function saveOfflineTransaction(transaction) {
  try {
    ...
    console.error(
      "[v0] Error saving to IndexedDB, falling back to localStorage:",
      error,
    );
...
export async function getOfflineTransactions() {
  try {
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onsuccess = () => {
        console.log(
          "[v0] Retrieved offline transactions:",
    ...
    console.error(
      "[v0] Error reading from IndexedDB, falling back to localStorage:",
      error,
    );
    const queue = JSON.parse(
      localStorage.getItem("offlineTransactions") || "[]",
...
export async function markTransactionSynced(id) {
  try {
    ...
    console.error(
      "[v0] Error updating IndexedDB, falling back to localStorage:",
      error,
    );
    const queue = JSON.parse(
      localStorage.getItem("offlineTransactions") || "[]",
    ...
    const updated = queue.map((item) =>
      item._offlineId === id ? { ...item, synced: true } : item,
    );
...
export async function clearSyncedTransactions() {
  try {
    ...
    console.error(
      "[v0] Error clearing IndexedDB, falling back to localStorage:",
      error,
    );
    const queue = JSON.parse(
      localStorage.getItem("offlineTransactions") || "[]",
