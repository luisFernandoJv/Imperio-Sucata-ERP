...
export const getTransactionById = async (id) => {
  try {
    ...
    console.error("[v0] Erro ao buscar transação por ID:", error);
    return null;
  }
};
...
export const deleteTransaction = async (id) => {
  try {
    ...
    // CORREÇÃO CRÍTICA: Reverter o estoque ao deletar uma transação de compra ou venda.
    // Antes desta correção, deletar uma compra deixava os kg somados no inventário
    // permanentemente. Ex: deletar uma compra de 6 kg de COBRE_MEL → inventário ficava
    // 6 kg acima do real. A reversão usa o tipo oposto para desfazer a operação original.
    if (material && (tipo === "compra" || tipo === "venda")) {
      try {
        const tipoReverso = tipo === "compra" ? "venda" : "compra";
        await updateInventoryQuantity(material, quantidade, tipoReverso);
        console.log(
          `[v0] Estoque revertido após deleção: ${material} ${tipo === "compra" ? "-" : "+"}${quantidade}kg`,
        );
      } catch (inventoryError) {
        console.error(
          "[v0] Erro ao reverter estoque após deleção (não crítico):",
          inventoryError,
        );
      }
    }
    ...
    console.log(
      "[v0] Daily report, live_summary e inventário atualizados após exclusão",
    );
  } catch (error) {
...
async function subtractFromReport(
  ...
  if (
    ...
    if (tipo === "venda") {
      ...
      // CORREÇÃO: quantidade comprada também precisa ser subtraída do materialStats
      // ao deletar/reverter uma compra. A omissão aqui fazia o relatório acumular
      // kg comprados fantasmas, divergindo do estoque real (ex: COBRE_MEL exibia
      // 88 kg comprados quando apenas 62 deveriam constar, causando diferença de 6 kg
      // no inventário — exibido como 20 kg em vez dos 26 corretos).
      matStats.quantidadeComprada = Math.max(
        0,
        (matStats.quantidadeComprada || 0) - quantidade,
      );
    }
  ...
  try {
    ...
    if (docSnap.exists()) {
      ...
      const initialInventory = {
        ...
        chumbo: { quantidade: 0, precoCompra: 4.5, precoVenda: 6.0 },
      };
...
export const updateInventoryQuantity = async (material, quantidade, tipo) => {
  try {
    ...
    // Lógica de cálculo
    // CORREÇÃO: Permitimos estoque negativo intencionalmente.
    // Math.max(0,...) mascarava divergências silenciosamente — ex: compra 88kg,
    // venda 62kg registrada antes → Math.max cortava para 0 em vez de -62,
    // depois somava 88 resultando em 88 em vez do correto 26kg.
    // Quantidade negativa passada pelo DataContext = operação de reversão:
    //   reverter compra: tipo="compra", quantidade < 0 → valorAtual + (neg) = subtrai
    //   reverter venda:  tipo="venda",  quantidade < 0 → valorAtual - (neg) = soma de volta
    const valorAtual = Number(currentInventory[material].quantidade) || 0;
    const qtd = Number(quantidade);
    let novaQuantidade;
    if (tipo === "compra") {
      novaQuantidade = valorAtual + qtd;
    } else if (tipo === "venda") {
      novaQuantidade = valorAtual - qtd;
    } else {
...
export const getTransactionsByPeriod = async (startDate, endDate) => {
  try {
    // CORREÇÃO: new Date("yyyy-MM-dd") interpreta como UTC meia-noite,
    // o que em UTC-3 equivale a 21h do dia anterior (off-by-one de timezone).
    // Construímos as datas como LOCAL para respeitar o fuso do usuário.
    let start, end;
    if (
...
export const getAggregatedReport = async (startDate, endDate) => {
  try {
    // CORREÇÃO: same timezone fix as getTransactionsByPeriod —
    // parse "yyyy-MM-dd" strings as local dates to avoid UTC off-by-one in UTC-3.
    let start, end;
    if (
      typeof startDate === "string" &&
      /^\d{4}-\d{2}-\d{2}$/.test(startDate)
...
export const getCustomerTransactions = async (customerId, customerName) => {
  try {
    // CORREÇÃO: Em vez de getAllTransactions() (sem limite, carrega tudo em memória)
    // e filtrar em JS, usamos queries diretas no Firestore com where().
    // Fazemos duas queries (por ID e por nome) e mesclamos sem duplicatas.

    const results = new Map();

    // Query 1: por clienteId (mais confiável, baseado em ID)
    if (customerId) {
      const qById = query(
        collection(db, TRANSACTIONS_COLLECTION),
    ...
    // Query 2: por nome do vendedor (fallback para transações sem clienteId)
    if (customerName) {
      const qByName = query(
        collection(db, TRANSACTIONS_COLLECTION),
        where("vendedor", "==", customerName),
        orderBy("data", "desc"),
      );
      const snapByName = await getDocs(qByName);
      snapByName.docs.forEach((docSnap) => {
    ...
    const customerTransactions = Array.from(results.values()).sort(
...
// ─────────────────────────────────────────────────────────────────────────────
// GESTÃO DINÂMICA DE MATERIAIS NO INVENTÁRIO
// ─────────────────────────────────────────────────────────────────────────────

/**
 * Adiciona um novo material ao documento de inventário no Firestore.
 * Se o material já existir, retorna false (use updateInventoryItem para editar).
 */
export const addMaterialToInventory = async (key, materialData) => {
  try {
    if (!key || typeof key !== "string" || key.trim() === "") {
      throw new Error("Chave do material inválida");
    }
    ...
    if (cleanKey in currentInventory) {
      return { success: false, reason: "already_exists" };
    }

    currentInventory[cleanKey] = {
      quantidade: Number(materialData.quantidade) || 0,
      precoCompra: Number(materialData.precoCompra) || 0,
      precoVenda: Number(materialData.precoVenda) || 0,
    };

    // Salvar limpo
    const inventoryToSave = {};
    for (const [k, v] of Object.entries(currentInventory)) {
      inventoryToSave[k] = {
        quantidade: Number(v.quantidade) || 0,
        precoCompra: Number(v.precoCompra) || 0,
    ...
    console.log(
      `[firebaseService] Material "${cleanKey}" adicionado ao inventário`,
    );
    return { success: true, key: cleanKey };
  } catch (error) {
    console.error("[firebaseService] Erro ao adicionar material:", error);
    throw error;
  }
};

/**
 * Remove um material do documento de inventário no Firestore.
 * Só remove se a quantidade for 0, por segurança.
 */
export const deleteMaterialFromInventory = async (key) => {
  try {
    const docRef = doc(db, INVENTORY_COLLECTION, "current");
    const docSnap = await getDoc(docRef);

    if (!docSnap.exists()) throw new Error("Inventário não encontrado");

    const currentInventory = normalizeInventory(docSnap.data());

    if (!(key in currentInventory)) {
      return { success: false, reason: "not_found" };
    }

    const qty = Number(currentInventory[key]?.quantidade) || 0;
    if (qty > 0) {
      return {
        success: false,
        reason: "has_stock",
    ...
    console.log(`[firebaseService] Material "${key}" removido do inventário`);
    return { success: true };
  } catch (error) {
    console.error("[firebaseService] Erro ao remover material:", error);
    throw error;
  }
};
