...
const Dashboard = ({ onQuickAction }) => {
  ...
  const calculateStats = () => {
    ...
    const margemLucro =
      totalCompras > 0
        ? ((totalVendas - totalCompras - totalDespesas) / totalCompras) * 100
        : 0;
    const crescimentoSemanal =
      vendasSemanaPassada > 0
        ? ((totalVendas - vendasSemanaPassada) / vendasSemanaPassada) * 100
        : 0;

    const ticketMedio =
      transactions.filter((t) => (t.tipo || t.type) === "venda").length > 0
        ? totalVendas /
          transactions.filter((t) => (t.tipo || t.type) === "venda").length
        : 0;
    ...
    transactions
      ...
      .forEach((t) => {
        materialCount[t.material] =
          (materialCount[t.material] || 0) + (t.quantidade || 0);
      });
  ...
  const calculatePaymentStats = () => {
    ...
    setPaymentStats({
      ...
      totalTransactions,
      pixPercentage:
        totalTransactions > 0 ? (stats.pix / totalTransactions) * 100 : 0,
      dinheiroPercentage:
        totalTransactions > 0 ? (stats.dinheiro / totalTransactions) * 100 : 0,
    });
  };
  ...
  const calculateWeeklyTrend = () => {
    ...
    for (let i = 6; i >= 0; i--) {
      ...
      const dayTransactions = transactions.filter(
        (t) => new Date(t.data).toDateString() === dateString,
      );

      const dayTotal = dayTransactions.reduce(
        (sum, t) => sum + (t.valorTotal || t.total || 0),
        0,
      );
    ...
    },
    {
      ...
      bgColor:
        stats.lucroTotal >= 0
          ...
          : "bg-gradient-to-br from-red-50 to-red-100",
      borderColor:
        stats.lucroTotal >= 0 ? "border-emerald-200" : "border-red-200",
    ...
    <div className="space-y-6 lg:space-y-8">
      <div className="text-center">
        ...
        <p className="text-gray-600 mb-4">
          Controle total do seu negócio de reciclagem
        </p>
        ...
        <div className="flex justify-center items-center gap-4 mb-4">
          ...
          >
            ...
            <span className="font-medium">
              {firebaseConnected ? "🟢 Sistema Online" : "🟡 Modo Offline"}
            </span>
          </motion.div>
            ...
            <Card className="p-4 border-orange-200 bg-gradient-to-r from-orange-50 to-yellow-50">
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                {lowStockAlerts.slice(0, 6).map((alert) => (
                  ...
                  >
                    {getMaterialIcon(alert.material)} {alert.material} (
            ...
            >
              ...
              >
                <div className="flex items-center justify-between mb-3">
                  <div
                    className={`p-2 lg:p-3 rounded-xl bg-white/60 shadow-sm`}
                  >
                    <Icon className={`h-5 w-5 lg:h-6 lg:w-6 ${stat.color}`} />
                  </div>
                  <div className="text-right">
                    <p
                      className={`text-xs lg:text-sm font-medium ${stat.changeColor}`}
                    >
                      {stat.change}
                    </p>
                  </div>
                ...
                <div>
                  ...
                  <p
                    className={`text-lg lg:text-xl font-bold ${stat.color} truncate`}
                  >
                    {stat.value}
                  </p>
            ...
            <div className="flex items-center justify-between p-4 bg-gradient-to-r from-blue-50 to-blue-100 rounded-lg border border-blue-200">
              ...
              <div className="text-right">
                <p className="font-bold text-blue-700 text-lg">
                  {formatCurrency(paymentStats.pix)}
                </p>
            ...
            <div className="flex items-center justify-between p-4 bg-gradient-to-r from-green-50 to-green-100 rounded-lg border border-green-200">
              ...
              <div className="text-right">
                <p className="font-bold text-green-700 text-lg">
                  {formatCurrency(paymentStats.dinheiro)}
                </p>
              ...
              <div className="flex items-center justify-between p-3 bg-gray-50 rounded-lg border border-gray-200">
                ...
                <div className="text-right">
                  <p className="font-bold text-gray-700">
                    {formatCurrency(paymentStats.outros)}
                  </p>
                </div>
              </div>
            )}
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-500">
                      {material.quantidade.toFixed(1)}kg
                    </p>
                  </div>
                </div>
                <div className="text-right">
                  <p className="font-bold text-sm">
                    {formatCurrency(material.valor)}
                  </p>
                  <p className="text-xs text-gray-500">
                    {material.transacoes} transações
                  </p>
                </div>
              </div>
            ))}
            ...
            {recentTransactions.slice(0, 6).map((transaction, index) => (
              ...
              >
                <div className="flex items-center space-x-3">
                  <div
                    className={`w-4 h-4 rounded-full ${getMaterialColor(transaction.material)}`}
                  ></div>
                  <div>
                    <div className="flex items-center gap-2">
                      <span
                    ...
                    <p className="text-xs text-gray-500">
                      {(transaction.quantidade || 0).toFixed(1)}kg •{" "}
                ...
                <div className="text-right">
                  <p
                    className={`font-bold ${transaction.tipo === "venda" ? "text-green-600" : "text-blue-600"}`}
                  >
        ...
        {showGoalEditor && (
          <div className="mb-4 p-4 bg-gray-50 rounded-lg">
            ...
            <Input
              ...
              value={monthlyGoal}
              onChange={(e) =>
                setMonthlyGoal(Number.parseFloat(e.target.value) || 0)
              }
              className="mt-2"
            />
            <Button
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4 pt-4 border-t">
            <div className="text-center">
              ...
              <p
                className={`text-2xl font-bold ${stats.crescimentoSemanal >= 0 ? "text-green-600" : "text-red-600"}`}
              >
            ...
            <div className="text-center">
              ...
              <p
                className={`text-2xl font-bold ${stats.margemLucro >= 0 ? "text-green-600" : "text-red-600"}`}
              >
