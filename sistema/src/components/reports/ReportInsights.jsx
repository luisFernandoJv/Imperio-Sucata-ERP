...
const ReportInsights = ({ transactions, stats, dateRange }) => {
  const insights = useMemo(() => {
    ...
    // Normalizar campos: suporta tanto OptimizedReports (totalVendas/lucroTotal)
    // quanto ProfessionalReports (totalSales/totalProfit)
    const totalSales = Number(stats.totalVendas ?? stats.totalSales ?? 0);
    const totalPurchases = Number(
      stats.totalCompras ?? stats.totalPurchases ?? 0,
    );
    const totalExpenses = Number(
      stats.totalDespesas ?? stats.totalExpenses ?? 0,
    );
    const totalProfit = Number(stats.lucroTotal ?? stats.totalProfit ?? 0);
    const totalTransactions = Number(
      stats.totalTransacoes ??
        stats.totalTransactions ??
        (stats.countVendas ?? 0) +
          (stats.countCompras ?? 0) +
          (stats.countDespesas ?? 0) ??
        transactions.length,
    );
    ...
    // Análise por material — com parsing seguro de valores numéricos
    const materialStats = {};
    transactions.forEach((t) => {
      ...
      if (t.tipo === "venda") {
        materialStats[t.material].vendas += valor;
        materialStats[t.material].quantidade += qtd;
      } else if (t.tipo === "compra") {
        materialStats[t.material].compras += valor;
      }
    ...
    // Oportunidades: alta margem, baixa participação
    const opportunities = Object.entries(materialStats)
      .map(([material, data]) => ({
        material,
        ...data,
        margem: data.vendas > 0 ? (data.lucro / data.vendas) * 100 : 0,
        participacao: totalSales > 0 ? (data.vendas / totalSales) * 100 : 0,
      }))
      .filter((m) => m.margem > 50 && m.participacao < 10 && m.vendas > 0)
      .sort((a, b) => b.margem - a.margem)[0];

    // Análise por dia da semana — com suporte a Firestore Timestamp
    const dayStats = {};
    transactions.forEach((t) => {
      let dateObj;
      if (t.data?.toDate) {
        dateObj = t.data.toDate();
      } else {
      ...
      dayStats[dayName].valor += Number(t.valorTotal) || 0;
    });
    ...
    // Detectar aumento de custo de compra por material
    ...
    compras.forEach((t) => {
      ...
      let dateObj = t.data?.toDate ? t.data.toDate() : new Date(t.data);
      avgPurchasePrice[t.material].total += Number(t.precoUnitario) || 0;
      avgPurchasePrice[t.material].count++;
      avgPurchasePrice[t.material].prices.push({
        price: Number(t.precoUnitario) || 0,
        date: dateObj,
      });
    });
    ...
    Object.entries(avgPurchasePrice).forEach(([material, data]) => {
      if (data.prices.length >= 3) {
        ...
        const olderItems = sorted.slice(0, -3);
        if (olderItems.length === 0) return;
        const older =
          olderItems.reduce((sum, p) => sum + p.price, 0) / olderItems.length;
        if (older === 0) return;
    ...
    // KPIs calculados a partir dos dados normalizados
    const margemLucro = totalSales > 0 ? (totalProfit / totalSales) * 100 : 0;
    const roi = totalPurchases > 0 ? (totalProfit / totalPurchases) * 100 : 0;
    const ticketMedio =
      totalTransactions > 0
        ? (totalSales + totalPurchases) / totalTransactions
        : 0;

    // Lucro médio diário — suporte a dateRange com Date ou string
    let diasPeriodo = 1;
    if (dateRange?.from && dateRange?.to) {
      ...
      diasPeriodo = Math.max(1, Math.ceil((to - from) / (1000 * 60 * 60 * 24)));
    }
    const lucroMedioDiario = totalProfit / diasPeriodo;
    ...
    return {
      ...
      busiestDay,
      costIncreaseAlert,
      kpis: { ticketMedio, margemLucro, roi, lucroMedioDiario },
      // Passando totalProfit normalizado para o resumo de performance
      totalProfit,
      totalTransactions,
    };
      ...
      <Card className="p-6 bg-gradient-to-br from-slate-50 to-gray-100 shadow-xl border-2 border-slate-300">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="bg-white rounded-lg p-4 border-2 border-slate-200">
            ...
            <div className="flex items-center gap-2">
              {insights.totalTransactions >= 50 ? (
                <>
                  <TrendingUp className="h-5 w-5 text-green-600" />
                  <span className="text-lg font-bold text-green-600">Alto</span>
                </>
              ) : insights.totalTransactions >= 20 ? (
                <>
