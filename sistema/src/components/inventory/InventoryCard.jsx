        ...
        {!isEditing && (
          <div className="absolute top-3 right-3 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity z-10">
            ...
            <Button
              ...
              title="Editar preços"
              className="h-8 w-8 rounded-full shadow-sm bg-white hover:bg-blue-50 hover:text-blue-600 border border-slate-200"
            >
            ...
            {onConfigure && (
              <Button
                variant="secondary"
                size="icon"
                onClick={() => onConfigure(material)}
                title="Configurar material"
                className="h-8 w-8 rounded-full shadow-sm bg-white hover:bg-purple-50 hover:text-purple-600 border border-slate-200"
              >
                <Settings className="h-3.5 w-3.5" />
              </Button>
            )}
            {/* Remover — somente materiais dinâmicos */}
            {onDelete && material.isDynamic && (
              <Button
                variant="secondary"
                size="icon"
                onClick={() => onDelete(material)}
                title="Remover material"
                className="h-8 w-8 rounded-full shadow-sm bg-white hover:bg-red-50 hover:text-red-600 border border-slate-200"
              >
            ...
            <>
              ...
              <div className="mb-6">
                <div className="flex justify-between items-baseline mb-2">
                  <span
                    className={`text-3xl font-extrabold tracking-tighter ${isNegative ? "text-rose-600" : "text-slate-800"}`}
                  >
                    {formatWeight(qtd).replace("kg", "")}
                ...
                <div className="h-2 w-full bg-slate-100 rounded-full overflow-hidden shadow-inner">
                  <motion.div
                    initial={{ width: 0 }}
                    animate={{
                      width: `${Math.min(Math.max((qtd / (minStock * 4)) * 100, 5), 100)}%`,
                    }}
              ...
              <div className="mt-auto grid grid-cols-2 gap-3 pt-4 border-t border-slate-100">
                ...
                <div className="col-span-2 mt-1">
                  <div
                    className={`flex items-center justify-between px-2 py-1.5 rounded-lg ${margin > 0 ? "bg-emerald-50 text-emerald-700" : "bg-slate-50 text-slate-600"}`}
                  >
